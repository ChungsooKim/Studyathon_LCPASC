...
if (exists("shinySettings")) {
  # run via TreatmentPatterns::launchResultsExplorer
  ...
  if (exists("pathResultsDirectory")) {
    ...
    stop(
      "Define location of outputFolder in pathResultsDirectory (folder ~/output including the different database results)"
    )
  }
}
...
# Fixing the labels
included_databases <-
  list.dirs(local, recursive = FALSE, full.names = FALSE)
names(included_databases) <-
  included_databases # optional: change with own custom names
...
all_targetcohorts <-
  unique(as.numeric(pathway_settings[pathway_settings$param == "targetCohortId", -1]))
names(all_targetcohorts) <-
  sapply(all_targetcohorts, function(c)
    cohorts$cohortName[cohorts$cohortId == c])
all_targetcohorts <- as.list(all_targetcohorts)

all_studynames <-
  unique(as.character(pathway_settings[pathway_settings$param == "studyName", -1]))
names(all_studynames) <-
  all_studynames # optional: change with own custom names
all_studynames <- as.list(all_studynames)

all_years <-
  list(
    "Entire study period" = "all",
    # todo: adjust to study period
...
suppressWarnings({
  for (d in included_databases) {
    ...
    try(characterization[[d]]  <-
          read.csv(paste0(local, "/", d, "/characterization/characterization.csv")), silent = TRUE)
    ...
    for (s in available_studies[available_studies != "characterization"]) {
      try({
        # Load summary counts
        file <-
          read.csv(paste0(local, "/", d, "/", s, "/", d , "_", s, "_summary_cnt.csv"))
        transformed_file <-
          data.table(
            year = character(),
            number_target = integer(),
            number_pathways = integer()
          )
        transformed_file <-
          rbind(transformed_file, list("all", file$N[file$index_year == "Number of persons in target cohort NA"], file$N[file$index_year == "Number of pathways (before minCellCount) in NA"]))
        
        for (y in all_years[-c(1)]) {
          try(transformed_file <-
                rbind(transformed_file, list(y, file$N[file$index_year == paste0("Number of persons in target cohort ", y)], file$N[file$index_year == paste0("Number of pathways (before minCellCount) in ", y)])),
              silent = TRUE)
        }
        
        transformed_file$perc <-
          round(
            transformed_file$number_pathways * 100.0 / transformed_file$number_target,
            1
          )
        ...
        summary_treated_d[[s]] <-
          file[grep("Percentage treated", file$index_year), ]
        
      }, silent = TRUE)
      
      # Load event cohorts classes file for available study settings
      try(summary_eventcohorts_d[[s]] <-
            read.csv(
              paste0(
                local,
                "/",
      ...
      try(summary_eventcohorts_year_d[[s]] <-
            read.csv(
              paste0(
                local,
                "/",
      ...
      try(duration_d[[s]] <-
            read.csv(paste0(local, "/", d, "/", s, "/", d , "_", s, "_duration.csv")), silent = TRUE)
    }
