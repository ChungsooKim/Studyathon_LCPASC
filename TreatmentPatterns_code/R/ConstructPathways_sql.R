...
constructPathways <- function(dataSettings, 
  ...
  #K 
  # Generate cohort afterwards to minimise code (then we get cohortIds)
  ...
  for (s in settings) {
    ...
    #K
    # This can be done like this as I will only have 1 analysis, if not, not very efficient
    ...
    #K
    full_cohorts <- data.table::as.data.table(
      cdm[[tableName]] %>%
        dplyr::filter(cohort_definition_id %in% c(targetCohortId,eventCohortIds)))
    
    colnames(full_cohorts) <- c("person_id", 
                                "cohort_id", 
                                "start_date", 
                                "end_date")  
...
doCreateTreatmentHistory <- function(
  ...
  if (includeTreatments == "startDate") {
    ...
    currentCohorts$start_date.x <- ifelse(
